...
<<<<<<< HEAD
    ...
    def _evaluate_coherent_scenarios(self, trend, price) -> Dict[str, Any]:
        """Generate scenarios probabilities based on trend"""
        if trend == "Bullish":
            p_bull = random.randint(65, 85)
            p_bear = random.randint(5, 15)
        elif trend == "Bearish":
            p_bull = random.randint(5, 15)
            p_bear = random.randint(65, 85)
        else:
            p_bull = random.randint(30, 45)
            p_bear = random.randint(30, 45)
            
        p_wait = 100 - p_bull - p_bear
        
        return {
            "bullish": {"probability": p_bull, "case": "Continuation of trend", "target": round(price * 1.05, 2)},
            "bearish": {"probability": p_bear, "case": "Reversal of trend", "target": round(price * 0.95, 2)},
            "wait": {"probability": p_wait, "case": "Neutral consolidation", "reason": "Lack of momentum"}
        }
    ...
    def _generate_coherent_news(self, trend, symbol):
        ...
        if is_opcvm:
             headlines = {
                "Bullish": ["Croissance soutenue des souscriptions sur ce fonds", "Bank Al-Maghrib maintient un taux directeur favorable", "Surperformance des actions marocaines dans le portefeuille"],
                "Bearish": ["Retraits importants observés sur les OPCVM obligataires", "Hausse surprise du taux directeur par BAM", "Baisse de la BVC impactant la valorisation du fonds"],
                "Neutral": ["Stabilité de la valeur liquidative en période de rumeurs macro", "Rééquilibrage tactique du portefeuille par les gérants", "Attente des résultats semestriels des valeurs bancaires marocaines"]
            }
             desc = f"Les actualités macro-économiques marocaines et les flux de capitaux justifient une perspective {trend.lower()} sur cette classe d'actifs."
        else:
            headlines = {
                "Bullish": ["Strong Earnings Beat Expectations", "Analyst Upgrades to Outperform", "Positive Macro Data Release"],
                "Bearish": ["Inflation Concerns Rise", "Missed Earnings & Weak Guidance", "Regulatory Headwinds Intensify"],
            ...
            desc = f"News flow is supporting a {trend.lower()} outlook."
            
        return {
            "impact": impact,
            "key_events": random.sample(headlines.get(trend, headlines["Neutral"]), 2),
            "description": desc,
            "overreaction_risk": "Low" if is_opcvm else "Medium"
        }
    ...
    def _make_coherent_decision(self, trend, scenarios, symbol=None):
        ...
        if trend == "Bullish":
            reason = "L'analyse montre une accumulation institutionnelle, une tendance haussière claire (MACD positif) et des fondamentaux économiques robustes, justifiant un achat à ce niveau." if is_opcvm else "Strong bullish structure with confirmed EMA alignment and positive momentum."
            return {
                "recommendation": "BUY",
                "confidence": scenarios["bullish"]["probability"],
                "reasoning": reason
            ...
            reason = "Les indicateurs signalent un essoufflement de la tendance (RSI suracheté, MACD décroisant). Il est préférable de vendre ou d'alléger la position face aux incertitudes du marché." if is_opcvm else "Bearish market structure, negative MACD, and price rejected at EMA resistance."
            return {
                "recommendation": "SELL",
                "confidence": scenarios["bearish"]["probability"],
                "reasoning": reason
            ...
            reason = "La valorisation actuelle ne montre pas de direction claire. L'attentisme est recommandé jusqu'à un franchissement de niveau clé." if is_opcvm else "Market is consolidating. Wait for a breakout of key levels before entering."
            return {
                "recommendation": "WAIT",
                "confidence": scenarios["wait"]["probability"],
                "reasoning": reason
